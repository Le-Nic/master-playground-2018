...
class LSTMCell(RNNCell):
    ...
    def __init__(self, num_units, seed=147):
    ...
    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or type(self).__name__):
            ...
            W_xh = tf.get_variable('W_xh',
                ...
                initializer=orthogonal_initializer(self.seed))
            W_hh = tf.get_variable('W_hh',
                [self.num_units, 4 * self.num_units],
                initializer=bn_lstm_identity_initializer(0.95, self.seed))
...
class BNLSTMCell(RNNCell):
    ...
    def __init__(self, num_units, training, seed=147):
        self.num_units = num_units
    ...
    def __call__(self, x, state, scope=None):
        with tf.variable_scope(scope or 'bn_lstm'):
            ...
            W_xh = tf.get_variable(
                'W_xh', [x_size, 4 * self.num_units], initializer=orthogonal_initializer(self.seed))
...
def bn_lstm_identity_initializer(scale, seed=147):
    def _initializer(shape, dtype=tf.float32, partition_info=None):
        ...
        t[:, :size] = orthogonal([size, size], seed)
        t[:, size * 2:size * 3] = orthogonal([size, size], seed)
        t[:, size * 3:] = orthogonal([size, size], seed)
        return tf.constant(t, dtype=dtype)

    return _initializer


def orthogonal_initializer(seed=147):
    def _initializer(shape, dtype=tf.float32, partition_info=None):
        return tf.constant(orthogonal(shape, seed), dtype)
    return _initializer
...
def batch_norm(x, name_scope, training, epsilon=1e-3, decay=0.999):
    ...
    with tf.variable_scope(name_scope):
        ...
        return tf.cond(tf.cast(training, tf.bool), batch_statistics, population_statistics)
