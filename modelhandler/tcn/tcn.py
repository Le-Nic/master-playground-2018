...
def attentionblock(x, counters, dropout, seed=147):
    ...
    with tf.variable_scope(name):
        # [N, L, k_size]
        key = tf.layers.dense(
            x, units=k_size, activation=None, use_bias=False,
            kernel_initializer=tf.random_normal_initializer(0, 0.01, seed=seed)
        )
        key = tf.nn.dropout(key, rate=dropout, seed=seed)
        ...
        query = tf.nn.dropout(query, rate=dropout, seed=seed)

        value = tf.layers.dense(
            x, units=v_size, activation=None, use_bias=False,
            kernel_initializer=tf.random_normal_initializer(0, 0.01, seed=seed)
        )
        value = tf.nn.dropout(value, rate=dropout, seed=seed)
    ...
    with tf.variable_scope(name):
        ...
        if init:
            ...
            V = tf.get_variable('V', [filter_size, int(x.get_shape()[-1]), num_filters],
                                tf.float32, tf.random_normal_initializer(0, 0.01, seed=seed),
                                trainable=True)
...
def TemporalBlock(input_layer, out_channels, filter_size, stride, dilation_rate, counters,
                  dropout, init=False, atten=False, use_highway=False, gated=False, seed=147):
    ...
    with tf.variable_scope(name):
        ...
        conv1 = weightNormConvolution1d(input_layer, out_channels, dilation_rate,
                                        ...
                                        init=init, gated=gated, seed=seed)
        ...
        out1 = tf.nn.dropout(conv1, noise_shape=noise_shape, rate=dropout, seed=seed)
        if atten:
            out1 = attentionblock(out1, counters, dropout, seed=seed)

        conv2 = weightNormConvolution1d(out1, out_channels, dilation_rate,
                                        filter_size, [stride], counters=counters,
                                        init=init, gated=gated, seed=seed)
        out2 = tf.nn.dropout(conv2, noise_shape=noise_shape, rate=dropout, seed=seed)
        if atten:
            out2 = attentionblock(out2, counters, dropout, seed=seed)

        # highway connections or residual connection
        residual = None
        if use_highway:
            W_h = tf.get_variable('W_h', [1, int(input_layer.get_shape()[-1]), out_channels],
                                  tf.float32, tf.random_normal_initializer(0, 0.01, seed=seed), trainable=True)
...
def TemporalConvNet(input_layer, num_channels, sequence_length, kernel_size=2,
                    ...
                    atten=False, use_highway=False, use_gated=False, seed=147):
    ...
    for i in range(num_levels):
        ...
        input_layer = TemporalBlock(
            input_layer, out_channels, kernel_size, stride=1, dilation_rate=dilation_size,
            counters=counters, dropout=dropout, init=init, atten=atten, gated=use_gated, seed=seed
        )
